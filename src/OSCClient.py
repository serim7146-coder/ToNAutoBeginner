...
送れるのは移動・視点と、ボタン系の入力（/input/UseRight など）。

ワールドUIについて（2026-09-25 に実測で訂正）: /input/UseRight で**裏の窓の
ワールドUIも押せる**。条件はフォーカスではなく「マウスカーソルがその窓の矩形の
中にあること」。カーソルをその窓の中へ置いて UseRight をパルス送信すると、裏の
まま Begin が押せた（別のウィンドウが上に重なっていても押せる）。対照として
カーソルを窓の外へ置くと押せない。前面化は不要。
以前ここには「UseRight ではワールドUIを押せない」と書いていた。2026-09-21 の
試行ではカーソルが窓の外にあったため押されず、アイテム（Emerald Coil）が
使われただけだった。押し方は WindowOperator.cursor_over_window() を参照。
"""
import socket
import struct
import subprocess
